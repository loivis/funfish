...
set __kubectl_commands get set describe create replace patch delete edit apply namespace logs rolling-update scale cordon drain uncordon attach exec port-forward proxy run expose autoscale rollout label annotate taint config cluster-info api-versions version explain convert completion

set __kubectl_resources all certificatesigningrequests csr clusterrolebindings clusterroles clusters componentstatuses cs configmaps configmap cm controllerrevisions cronjobs customresourcedefinition crd daemonsets ds deployments deployment deploy endpoints ep events ev horizontalpodautoscalers hpa ingresses ingress ing jobs limitranges limits namespaces namespace ns networkpolicies netpol nodes node no persistentvolumeclaims pvc persistentvolumes pv poddisruptionbudgets pdb podpreset pods pod po podsecuritypolicies psp podtemplates replicasets rs replicationcontrollers rc resourcequotas quota rolebindings roles secrets secret serviceaccounts sa services service svc statefulsets sts storageclass storageclasses sc

set __kubectl_config_subcommands current-context delete-cluster delete-context get-clusters get-contexts rename-context set set-cluster set-context set-credentials unset use-context view
...
set -q FISH_KUBECTL_COMPLETION_TIMEOUT
or set FISH_KUBECTL_COMPLETION_TIMEOUT 5s
...
function __fish_print_resource -d 'Print a list of resources' -a resource
    ...
    test $all_ns -eq 1
    and __fish_kubectl get "$resource" -o name --all-namespaces | string replace -r '(.*)/' ''
    and return

    set -l namespace (__fish_kubectl_get_namespace)
    test -z "$namespace"
    and __fish_kubectl get "$resource" -o name | string replace -r '(.*)/' ''
    and return

    __fish_kubectl --namespace "$namespace" get "$resource" -o name | string replace -r '(.*)/' ''
end
